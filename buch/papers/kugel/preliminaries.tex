...
However, if the reader is already familiar with the following concepts: inner
product space, Hilbert space, Fourier series, Laplacian operator and
...
this new Hilbert space of lemma \ref{kugel:thm:sqint-hilbert}.
...
$C(\mathbb{R}/\mathbb{Z}; \mathbb{C})$, from which follows the interesting
...
construction of the proof of the following theorem.
...
Another way common to read Fourier's theorem stated above, is that we begin
with a finite linear combination to approximate $f(x)$, then by continuing to
add more and more terms (infinitely many) it will eventually become
indistinguishable from $f(x)$ (under the $L^2$ norm). Finally, there is also
another perspective for the Fourier theory typically used in engineering,
where we interpret the result as a relationship between two functions: one in
the ``time-domain'' $f(t)$ and the \emph{spectrum} in the ``frequency
domain''.
