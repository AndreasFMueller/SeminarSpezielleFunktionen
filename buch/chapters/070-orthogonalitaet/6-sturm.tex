...
%\kopfrechts{Das Sturm-Liouville-Problem}
...
Der Vektor $v$ heisst {\em verallgemeinerter Eigenvektor}
...
\]
maximiert, dann ist er ein verallgemeinerter Eigenvektor für die Matrizen $A$
und $B$.
\end{satz}
...
2v^tMu.
\end{align*}
Dies wenden wir jetzt auf den Quotienten $\lambda(v+tu)$ an und erhalten
\begin{align*}
...
\notag
\end{align}
Um zu erreichen, dass der Operator selbstadjungiert wird, muss 
...
\label{buch:orthogonalitaet:subsection:H}}
...
gewöhnliches Eigenwertproblem für den Operator $\tilde{A}=B^{-1}A$
...
Der Operator ist definiert auf Funktionen des im
Abschnitt~\ref{buch:orthogonalitaet:subsection:H}
