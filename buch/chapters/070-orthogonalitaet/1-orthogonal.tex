...
\kopfrechts{Orthogonale Funktionenfamilien}
...
\]
ein Skalarprodukt.
Dazu gehört die Norm $\|f\|$ mit $\|f\|^2 = \langle f,g\rangle$.
\end{satz}
...
und
$\|f\|_w$ mit $\|f\|_w^2 = \langle f,g\rangle_w$ die zugehörige $w$-Norm.
\index{<,>@$\langle\;\,,\;\rangle$}%
...
Falls es genügt, eine orthogonale Basis zu finden, kann 
auf die Normierung
...
Wir nennen eine solche Familie orthogonal, wie die folgende Definition
...
\end{cases}
\end{align*}
Der Zähler $t^{\alpha-2a+1}$ divergiert genau dann für $t\to 0$,
...
dann hat $f(x)$ höchstens einen Pol der Ordnung $<\frac12(\alpha+1)$
bei $\xi$.
\item
Ist $\xi\in[a,b]$ eine Polstelle vom Grad $a$ der Funktion $w(x)$,
dann hat $f$ eine Nullstelle mindestens from Grad $\frac12(\alpha+1)$
an der Stelle $\xi$.
\end{enumerate}
\end{lemma}
...
Funktionen müssen daher ein Nullstelle mindestens vom Grad
