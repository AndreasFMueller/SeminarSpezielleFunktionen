...
Der Begriff der Summe einer Potenzreihe ist sinnvoll, wenn die Terme höheren
...
denn normalerweise wird bei Polynomen das Verhalten von den
Termen höheren Grades dominiert.
Sind die $a_k$ gross, muss entsprechend der Betrag $|z-z_0|^k$ klein
sein, damit die Reihe immer noch konvergiert.
...
\label{buch:potenzen:potenzreihen:eqn:geom}
\end{equation}
...
Die Reihe~\eqref{buch:potenzen:potenzreihen:eqn:geom}
ist konvergent für $|z|<1$ und divergent für $|z|\ge 1$.
...
Wenn $|a_k|\le b_k$ ist für fast alle $k$, dann ist die Reihe
\(
\sum_{k=}^\infty a_k
\)
absolut konvergent.
...
ist das Reststück der Reihe ab Index $N$ wieder
durch eine konvergente geometrische Reihe majorisiert.
\end{proof}
...
Das Quotienten- und das Wurzelkriterium ist auf beliebige Reihen
...
%
% Konvergenzradius der abgeleiteten Reihe
%
...
Ableitungen bis zur Ordnung $k$ beider Funktionen in $z_0$ übereinstimmen.
...
Das {\em Taylor-Polynom} vom Grad $n$ von $f(z)$ an der Stelle
$z_0$ ist die Summe
\begin{equation}
...
konvergiert.
Das Beispiel~\ref{buch:funktionentheorie:beispiel:nichtanalytisch}
auf
...
Da aber Potenzreihen gliedweise differenziert und integriert werden
