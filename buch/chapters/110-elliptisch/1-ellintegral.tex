...
Wurzel im Integranden von \eqref{buch:elliptisch:def:allgemein}
ausklammern und damit das Integral in eine Form bringen, wo $p(x)$
höchstens zweiten Grades ist.
Solche Integrale lassen sich mit der Methode von
Abschnitt~\ref{buch:integral:subsection:rxy}
berechnen.
...
Neue spezielle Funktionen ergeben sich, wenn wir die Integralgrenzen
per Konvention festlegen und damit eine Stammfunktion auswählen.
...
%
% Jacobi- und Legendre-Normalform
%
...
\]
werden die Funktionen $K(k)$, $E(k)$ und $\Pi(n,k)$ in die Form
\begin{align*}
K(k)
&=
\int_0^{\frac{\pi}2}
...
%
% Vollständige elliptische Integrale als hypergeometrische Funktionen
%
...
Binomialreihe in eine hypergeometrische Reihe umgeformt werden.
...
\begin{matrix}\frac12,\frac12\\1\end{matrix};k^2
\biggr)
\]
ausdrücken.
\end{satz}
...
mit Halbachsen $a$ und $b$, $a\le b$, auf ein vollständiges elliptisches
...
der Quotient $\varepsilon=e/b$ wird die {\em numerische Exzentrizität}
der Ellipse genannt.
In der Umfangsformel ist $k = \varepsilon$.

Das Integral~\eqref{buch:elliptisch:eqn:umfangellipse} erhält mit
diesen Bezeichungen die Form
\[
U
=
...
Für den Umfang der Ellipse finden wir somit die Formel
\[
U
=
4b E(k)
...
numerischer Exzentrizität $\varepsilon$ und grosser Halbachse $1$.
Für den extremen Wert $\varepsilon=0$ entsteht der Umfang eines Kreises,
also $E(0)=\frac{\pi}2$.
...
Stellen der Kreiszahl $\pi$, siehe Aufgabe~\ref{buch:elliptisch:aufgabe:5}.
...
\frac{a-b}{2^{n+1}}.
\]
Dies zeigt jedoch nur, dass die Konvergenzgeschwindigkeit mindestens ein
...
\]
ergibt.
Die Ableitung von $t$ nach $t_1$ kann man auch aus
...
Auch $\cos^2 t_1$ muss durch $1-\sin^2t_1$ ersetzt werden.
...
\end{table}
In diesem Abschnitt soll als Zahlenbeispiel $K(k)$ für $k=\sqrt{2}/2$
berechnet werden.
...
\index{Jacobi-Form}%
\index{Legendre-Form}%
mit dem Parameter $\vartheta$, gegeben durch
$\sin \vartheta=x$.
...
zeigt Graphen der unvollständigen elliptischen Integrale erster
und zweiter Art für verschiedene Werte des Parameters.
...
haben nur positive reelle Werte.
...
dabei aber möglicherweise komplexe Werte, weil die Radikanden in den
Integralen negativ werden.
Die Schwierigkeit dabei ist, dass die Quadratwurzel nicht eindeutig ist.
Welcher Wert der im Zusammenhang richtige ist, hängt davon ab, auf
welchem Weg in der komplexen Ebene wir dorthin kommen.
...
$3\frac{\pi}{2}$ und $2\pi$ wählen und für die Wurzel durch zwei teilen.
\item
...
Die obere Halbebene wird auf ein Rechteck der Breite $2K(k)$ abgebildet,
...
\label{buch:elliptisch:ellintegral:eqn:tk}
\end{equation}
...
\]
der Substitution~\eqref{buch:elliptisch:ellintegral:eqn:tk}
...
% Reelle Argument > 1/k
%
\subsubsection{Reelle Argumente $> 1/k$}
Für Argumente $x> 1/k$ sind beide Faktoren im Integranden des 
...
\label{buch:elliptisch:ellintegral:eqn:FiK}
\end{equation}
berechnen.
Dazu wenden wir die Variablentransformation
\[
y=\frac{1}{kt}\quad\Leftrightarrow\quad t=\frac{1}{ky}
\qquad\text{mit}\qquad
\frac{dt}{dy} = -\frac{1}{ky^2}
\]
auf das Integral~\eqref{buch:elliptisch:ellintegral:eqn:FiK}
an und erhalten
\begin{align*}
\int_x^\infty \frac{dt}{\sqrt{(1-t^2)(1-k^2t^2)}}
&=
...
Mit $a+b=2a+\varepsilon$ kann man $b$ aus der Formel eliminieren und erhält
