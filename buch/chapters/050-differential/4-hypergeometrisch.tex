...
\kopfrechts{Hypergeometrische Differentialgleichung}
...
=0.
\end{align*}
Für $k\ge 2$ erhalten wir durch Koeffizientenvergleich
\begin{align*}
0
&=
...
\end{align}
finden.
Setzt man $a_0=1$, ist die zweite Lösung also wieder eine
...
\index{Differentialgleichung!Exponentialfunktion}%
...
Wir fassen diese Resultat wie folgt zusammen.
\begin{satz}
...
Die geraden oder ungeraden Funktionen, die in früheren Beispielen
...
Dies wird einfacher, wenn wir das Gleichungssysteme
mit der Matrix
\begin{equation*}
\begin{pmatrix}
  g(x)\\
...
w''(sx^2)
\end{pmatrix}
\qquad\text{mit der Inversen}\qquad
A_g^{-1}
=
...
\end{pmatrix}
\end{equation*}
in Matrixschreibweise darstellen.
...
Differentialgleichung wiedergewonnen werden können.
...
%
% Differentialgleichung der Tschebyscheff-Polynome
%
...
\label{buch:differential:tschebyscheff:Tdgl}
\end{equation}
sie heisst die {\em Tschebyscheff-Differentialgleichung}.

%
% Tschebyscheff-Differentialgleichung und hypergeometrische
% Differentialgleichung
%
...
Der Koeffizient der zweiten Ableitung
in der Tschebyscheff-Dif\-fe\-ren\-tial\-glei\-chung
hat jedoch die Nullstellen $\pm 1$, während es in
...
\end{aligned}
\]
den gleichen Werten, die auch das Tschebyscheff-Polynom $T_n(x)$ annimmt.
Es folgt daher
\begin{equation}
T_n(x)
=
